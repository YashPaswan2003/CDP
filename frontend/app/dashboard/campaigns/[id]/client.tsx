...
export function CampaignDetailClient() {
  ...
  const kpiCards = [
    { label: "Spend", value: formatCurrency(campaign.spent, currency), change: kpiChanges.spend },
    { label: "Revenue", value: formatCurrency(campaign.revenue, currency), change: kpiChanges.revenue },
    { label: "ROAS", value: `${campaign.roas.toFixed(1)}x`, change: kpiChanges.roas },
    { label: "CTR", value: `${(campaign.ctr * 100).toFixed(1)}%`, change: kpiChanges.ctr },
    { label: "CPC", value: formatCurrency(campaign.cpc, currency), change: kpiChanges.cpc, invertColor: true },
    { label: "Conversions", value: formatNumber(campaign.conversions), change: kpiChanges.conversions },
  ];
    ...
    <div className="min-h-screen bg-gray-950 text-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
        <div className="space-y-3">
          <button onClick={() => router.push("/dashboard")} className="flex items-center gap-2 text-gray-400 hover:text-white text-sm font-medium transition-colors">
            <ArrowLeft className="w-4 h-4" /> Back to Portfolio
          </button>
          <div className="flex flex-wrap items-center gap-3">
            <h1 className="text-2xl font-bold text-white font-mono">{campaign.name}</h1>
            <span className="text-xs font-medium px-2.5 py-1 rounded-full bg-blue-500/15 text-blue-400 border border-blue-500/30">{platformLabel[campaign.platform] || campaign.platform}</span>
            <span className={`text-xs font-medium px-2.5 py-1 rounded-full border capitalize ${statusColor[campaign.status] || statusColor.ended}`}>{campaign.status}</span>
            <span className="text-sm text-gray-400">Budget: {formatCurrency(campaign.budget, currency)}</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          {kpiCards.map((kpi) => {
            ...
            return (
              <div key={kpi.label} className="bg-gray-900 border border-gray-800 rounded-xl p-4 space-y-2">
                ...
                <div className={`flex items-center gap-1 text-xs font-medium ${isPositive ? "text-green-400" : "text-red-400"}`}>
                  ...
                  <span>{isPositive ? "+" : "-"}{changeAbs.toFixed(0)}%</span>
                </div>
              </div>
            );
          })}
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-6 space-y-4">
          <h2 className="text-lg font-bold text-white">Campaign Performance Over Time</h2>
          {chartData.length > 0 ? (
            <ResponsiveContainer width="100%" height={320}>
              <RLineChart data={chartData}>
                ...
                <Tooltip contentStyle={{ backgroundColor: "#111827", border: "1px solid #374151", borderRadius: "0.5rem", color: "#f9fafb" }} />
                <Legend />
                ...
                <Line type="monotone" dataKey="Conversions" stroke="#F59E0B" strokeWidth={2} dot={false} activeDot={{ r: 4 }} />
              </RLineChart>
            </ResponsiveContainer>
          ) : (
            <div className="flex items-center justify-center h-64 text-gray-500">No daily metrics available for this campaign.</div>
          )}
        </div>

        <div className="bg-gray-900 border border-gray-800 rounded-xl p-6 space-y-4">
          <h2 className="text-lg font-bold text-white">Compare with Similar Campaigns</h2>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                <tr className="bg-blue-500/10 border-b border-gray-800">
                  <td className="py-3 px-4 font-medium text-white flex items-center gap-2"><Star className="w-4 h-4 text-amber-400 fill-amber-400" />{campaign.name}</td>
                  <td className="text-right py-3 px-4 text-white font-mono">{formatCurrency(campaign.spent, currency)}</td>
                  <td className="text-right py-3 px-4 text-white font-mono">{formatCurrency(campaign.revenue, currency)}</td>
                  <td className="text-right py-3 px-4 text-white font-mono">{campaign.roas.toFixed(1)}x</td>
                  <td className="text-right py-3 px-4 text-white font-mono">{(campaign.ctr * 100).toFixed(1)}%</td>
                  <td className="text-right py-3 px-4 text-white font-mono">{formatNumber(campaign.conversions)}</td>
                </tr>
                {similarCampaigns.map((c) => (
                  <tr key={c.id} className="border-b border-gray-800/50 hover:bg-gray-800/30 transition-colors">
                    <td className="py-3 px-4 text-gray-300">{c.name}</td>
                    <td className="text-right py-3 px-4 text-gray-300 font-mono">{formatCurrency(c.spent, currency)}</td>
                    <td className="text-right py-3 px-4 text-gray-300 font-mono">{formatCurrency(c.revenue, currency)}</td>
                    <td className="text-right py-3 px-4 text-gray-300 font-mono">{c.roas.toFixed(1)}x</td>
                    <td className="text-right py-3 px-4 text-gray-300 font-mono">{(c.ctr * 100).toFixed(1)}%</td>
                    <td className="text-right py-3 px-4 text-gray-300 font-mono">{formatNumber(c.conversions)}</td>
                  </tr>
                ))}
                {similarCampaigns.length === 0 && (
                  <tr><td colSpan={6} className="py-6 px-4 text-center text-gray-500">No similar campaigns found on this platform.</td></tr>
                )}
              </tbody>
            </table>
          </div>
