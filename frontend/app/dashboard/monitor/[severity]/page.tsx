...
export default function MonitorDrilldownPage() {
  ...
  const filteredFlags =
    platformFilter === "all"
      ? flags
      : flags.filter((f) => f.platform === platformFilter);
    ...
    <div className="space-y-6">
      ...
      <div>
        <Link
          ...
          className="text-sm text-text-secondary hover:text-text-primary flex items-center gap-1 mb-3 transition-colors"
        >
        ...
        <div className="flex items-center gap-3">
          <div
            className={`w-10 h-10 rounded-full ${config.bgClass} border ${config.borderClass} flex items-center justify-center`}
          >
          ...
          <div>
            ...
            <p className="text-text-secondary">
              {filteredFlags.length} item{filteredFlags.length !== 1 ? "s" : ""}{" "}
      ...
      {loading && (
        <div className="space-y-3">
          {[...Array(3)].map((_, i) => (
            <div
              key={i}
              className="animate-pulse bg-surface-elevated h-28 rounded-lg"
            />
          ))}
        </div>
      )}
      ...
      {!loading && !error && filteredFlags.length === 0 && (
        <div className="card text-center py-12">
          ...
          <p className="text-sm text-text-secondary mt-1">
            {platformFilter !== "all"
              ? `No ${config.label.toLowerCase()} items for the selected platform.`
              : "Great news — nothing to act on here."}
          </p>
        </div>
      )}
      ...
      {!loading && !error && filteredFlags.length > 0 && (
        <div className="space-y-3">
          {filteredFlags.map((flag, i) => (
            <div
              key={`${flag.metric}-${i}`}
              className={`card border ${config.borderClass} ${config.bgClass}`}
            >
              <div className="flex items-start gap-4">
                <Icon
                  className={`w-5 h-5 text-${config.color}-400 flex-shrink-0 mt-1`}
                />
                <div className="flex-1 min-w-0">
                    ...
                    <div>
                      ...
                      {flag.campaign_name && (
                        <p className="text-sm text-text-secondary">
                          {flag.campaign_name}
                          {flag.client_name && ` — ${flag.client_name}`}
                        </p>
                      )}
                    </div>
                    <span
                      className={`px-2 py-0.5 rounded text-xs font-semibold ${config.badgeClass}`}
                    >
                        ...
                        <div>
                          ...
                          <p
                            className={`text-lg font-bold ${
                              flag.current < flag.previous
                                ? "text-red-400"
                                : "text-emerald-400"
                            }`}
                          >
                            {(
                              ((flag.current - flag.previous) / flag.previous) *
                              100
                            ).toFixed(1)}
                            %
                          </p>
                  ...
                  {flag.entities.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 mt-3">
                      ...
                      {flag.entities.length > 5 && (
                        <span className="text-xs text-text-secondary">
                          +{flag.entities.length - 5} more
                        </span>
                      )}
                    </div>
                  )}
                  ...
                  <div className="flex flex-wrap gap-2 mt-4">
                    ...
                    {flag.platform && platformLinks[flag.platform] && (
                      <Link
                        href={platformLinks[flag.platform]}
