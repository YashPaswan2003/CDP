...
import { getMetaPlacements } from "@/lib/mockData";
...
export default function MetaPlacement() {
  const { selectedAccount } = useAccount();
  const pathname = usePathname();
  const [rowsPerPage, setRowsPerPage] = useState(10);
  const data = getMetaPlacements();

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-text-primary mb-1">Meta - Placements</h1>
      ...
      <ChartContainer title="Placements">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-700">
                  ...
                  <th className="text-left py-2 px-3 font-medium text-text-secondary">Surface</th>
                  ...
                  <th className="text-right py-2 px-3 font-medium text-text-secondary">Reach</th>
                  <th className="text-right py-2 px-3 font-medium text-text-secondary">Frequency</th>
              ...
              <tbody>
                {data.slice(0, rowsPerPage).map((row, idx) => (
                  <tr key={idx} className="border-b border-slate-800 hover:bg-slate-800/50">
                    <td className="py-2 px-3 text-text-primary">{row.placement}</td>
                    <td className="py-2 px-3 text-text-secondary capitalize">{row.surface.replace(/_/g, " ")}</td>
                    ...
                    <td className="py-2 px-3 text-right">{row.reach.toLocaleString()}</td>
                    <td className="py-2 px-3 text-right">{row.frequency.toFixed(2)}</td>
