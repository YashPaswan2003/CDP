...
import { getPMaxChannels } from "@/lib/mockData";
...
export default function GoogleAdsChannels() {
  const { selectedAccount } = useAccount();
  const pathname = usePathname();
  const [rowsPerPage, setRowsPerPage] = useState(10);
  const data = getPMaxChannels();

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-text-primary mb-1">Google Ads - Channels (PMax)</h1>
      ...
      <ChartContainer title="PMax Channels">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-700">
                  <th className="text-left py-2 px-3 font-medium text-text-secondary">Channel</th>
                  ...
                  <th className="text-right py-2 px-3 font-medium text-text-secondary">Revenue</th>
              ...
              <tbody>
                {data.slice(0, rowsPerPage).map((row, idx) => (
                  <tr key={idx} className="border-b border-slate-800 hover:bg-slate-800/50">
                    <td className="py-2 px-3 text-text-primary">{row.channel}</td>
                    ...
                    <td className="py-2 px-3 text-right">{row.cvr.toFixed(2)}%</td>
                    <td className="py-2 px-3 text-right">{formatCurrency(row.spend, selectedAccount?.currency)}</td>
                    <td className="py-2 px-3 text-right">{formatCurrency(row.revenue, selectedAccount?.currency)}</td>
