...
export default function GoogleAdsSQR() {
  ...
  useEffect(() => {
    setData(getSearchTerms());
  }, []);

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-text-primary mb-1">Google Ads - SQR</h1>
      ...
      <ChartContainer title="Search Query Report">
        <div className="space-y-4">
          <div className="flex justify-between items-center">
            <p className="text-sm text-text-secondary">
              Showing {Math.min(rowsPerPage, data.length)} of {data.length} search terms
            </p>
            <select
              value={rowsPerPage}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-700">
                  <th className="text-left py-2 px-3 font-medium text-text-secondary">Search Term</th>
              ...
              <tbody>
                {data.slice(0, rowsPerPage).map((row, idx) => (
                  <tr key={idx} className="border-b border-slate-800 hover:bg-slate-800/50">
                    <td className="py-2 px-3 text-text-primary">{row.keyword}</td>
                    <td className="py-2 px-3 text-text-secondary">{row.matchType}</td>
                    ...
                    <td className="py-2 px-3 text-right text-text-primary">{row.ctr.toFixed(2)}%</td>
                    <td className="py-2 px-3 text-right text-text-primary">{formatCurrency(row.cpc, selectedAccount?.currency)}</td>
