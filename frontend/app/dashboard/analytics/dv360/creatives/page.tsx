...
export default function DV360Creatives() {
  const { selectedAccount } = useAccount();
  const pathname = usePathname();
  const [rowsPerPage, setRowsPerPage] = useState(10);
  const creatives = getCreatives().filter((c) => c.platform === "dv360");

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-text-primary mb-1">DV360 - Creatives</h1>
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center">
            <p className="text-sm text-text-secondary">
              Showing {Math.min(rowsPerPage, creatives.length)} of {creatives.length} creatives
            </p>
            <select
              value={rowsPerPage}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-700">
                  ...
                  <th className="text-left py-2 px-3 font-medium text-text-secondary">Size</th>
              ...
              <tbody>
                {creatives.slice(0, rowsPerPage).map((creative) => (
                  <tr key={creative.creativeId} className="border-b border-slate-800 hover:bg-slate-800/50">
                    ...
                    <td className="py-2 px-3 text-text-secondary text-xs">{creative.size}</td>
