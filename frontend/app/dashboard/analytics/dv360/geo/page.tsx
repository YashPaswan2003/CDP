...
import { getGeoData } from "@/lib/mockData";
...
export default function DV360Geo() {
  const { selectedAccount } = useAccount();
  const pathname = usePathname();
  const [rowsPerPage, setRowsPerPage] = useState(10);
  const [selectedState, setSelectedState] = useState("All States");
  const data = getGeoData().filter((r) => r.platform === "dv360");

  // Extract unique states and sort
  const states = Array.from(new Set(data.map((r) => r.state))).sort();
  const filteredData = selectedState === "All States" ? data : data.filter((r) => r.state === selectedState);

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-text-primary mb-1">DV360 - Geo Performance</h1>
      ...
      <ChartContainer title="Geo Performance">
        <div className="space-y-4">
          <div className="flex justify-between items-center">
            <div>
              <label className="text-sm text-text-secondary mr-3">Filter by State:</label>
              <select
                value={selectedState}
          ...
          <p className="text-sm text-text-secondary">Showing {Math.min(rowsPerPage, filteredData.length)} of {filteredData.length}</p>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-700">
                  <th className="text-left py-2 px-3 font-medium text-text-secondary">City</th>
                  <th className="text-left py-2 px-3 font-medium text-text-secondary">State</th>
              ...
              <tbody>
                {filteredData.slice(0, rowsPerPage).map((row, idx) => (
                  <tr key={idx} className="border-b border-slate-800 hover:bg-slate-800/50">
                    <td className="py-2 px-3 text-text-primary">{row.city}</td>
                    <td className="py-2 px-3 text-text-secondary">{row.state}</td>
                    ...
                    <td className="py-2 px-3 text-right">{formatCurrency(row.cpc, selectedAccount?.currency)}</td>
