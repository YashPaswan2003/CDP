...
export default function DV360Placement() {
  const { selectedAccount } = useAccount();
  const pathname = usePathname();
  const [rowsPerPage, setRowsPerPage] = useState(10);
  const data = getPlacements().filter((p) => p.placementType === "youtube" || p.placementType === "display");

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-text-primary mb-1">DV360 - Placements</h1>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-700">
                  ...
                  <th className="text-right py-2 px-3 font-medium text-text-secondary">Views</th>
                  <th className="text-right py-2 px-3 font-medium text-text-secondary">VTR</th>
              ...
              <tbody>
                {data.slice(0, rowsPerPage).map((row, idx) => (
                  <tr key={idx} className="border-b border-slate-800 hover:bg-slate-800/50">
                    ...
                    <td className="py-2 px-3 text-text-secondary capitalize">{row.placementType}</td>
                    ...
                    <td className="py-2 px-3 text-right">{row.views.toLocaleString()}</td>
                    <td className="py-2 px-3 text-right">{row.vtr ? row.vtr.toFixed(2) + "%" : "-"}</td>
