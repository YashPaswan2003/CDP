...
// Helper function to format spend as Lakhs
const formatSpend = (num: number): string => {
            ...
            <thead>
              <tr>
                ...
                {headerConfigs.map(({ key, label }) => (
                  ...
                  >
                    {label} {sortKey === key ? (sortDir === "desc" ? "↓" : "↑") : <span className="opacity-40">↕</span>}
                  </th>
                ))}
              </tr>
            </thead>
            <tbody>
              {tableRows.map((row, i) => (
                <tr key={i} className="border-b border-[#E5E7EB] hover:bg-[#F7F8FA] transition-colors">
                  ...
                  <td className="px-4 py-3 text-right text-[#1F2937]">{formatSpend(row.spend)}</td>
                  <td className="px-4 py-3 text-right text-[#1F2937] font-medium">{formatROAS(row.roas)}</td>
                </tr>
              ))}
            </tbody>
          </table>
